...
class SettingsRepository(
    ...
    fun save(settings: Settings) {
        factory.ofy().save().entities(settings).now()
    }
    ...
    fun load(name: String?): Settings? =
        factory.ofy().load().type(Settings::class.java).id(name).now()
    ...
    fun find(freq: EmailFrequency): List<Settings> =
        factory.ofy().load().type(Settings::class.java).filter(Settings::emailFrequency.name, freq).list()
